...
class Game4Client extends Game4Game {
    constructor() {
        super()
    ...
    interpolateEntities() {
        ...
        var now = +new Date();
        var render_timestamp = now - (1000.0 / 30)//server.update_rate);
...
class Game4Server extends Game4Game {
    constructor(server) {
        super()
        this.server = server
...
class Game4 extends Cartridge {
    constructor(type = 0, server) {
        ...
        if (type == 1) {
            ...
            this.part = new Game4Server(server)
        }
    }
